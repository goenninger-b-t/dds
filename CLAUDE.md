...
- Never introduce a "Claude" reference — or any AI-assistant attribution — into any repository file except this `CLAUDE.md` agent-config file itself (so: `README.md`, `docs/` incl. `docs/wiki/` and ADRs, `src/`, `scripts/`, `Makefile`, `.github/`, …). Cite the operating contract neutrally (e.g. "the operating contract §N"), never by the `CLAUDE.md` filename. Extends the global no-AI-attribution rule (commits/PRs) to the entire repo. *(Owner directive, 2026-06-07.)*
