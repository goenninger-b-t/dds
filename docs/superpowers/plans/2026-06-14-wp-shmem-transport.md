...
## REVISION 2026-06-14 (during execution) — notification = pthread PROCESS_SHARED condvar
Probing during Phase B overturned two plan assumptions; the owner ratified the fix. **These supersede the `sem-*` parts of Phase B, the C1 header layout, and the D notification:**
1. **`shm_open` create needs `cffi:foreign-funcall-varargs`.** On arm64 the `mode` arg is variadic; a plain `foreign-funcall` passes a garbage mode → EACCES on later attach. Verified: varargs create → attach + cross-mapping read-back works. (`shm-attach` is 2-arg, non-variadic — plain funcall is fine.)
2. **Notification is a `PTHREAD_PROCESS_SHARED` mutex+cond IN the segment — NOT named semaphores.** `sem_open` returns EINVAL from the Lisp runtime on macOS arm64 (variadic `mode`/`value`; verified via CFFI plain+varargs+`sb-alien`; works from C). pthread primitives are all non-variadic → work from Lisp on macOS+Linux; libpthread is already linked (no new dep). **Remove the `sem-*` primitives + exports** (they are dead); **add** `pshared-mutex-init`/`pshared-cond-init`/`pshared-lock`/`pshared-unlock`/`pshared-cond-wait`/`pshared-cond-signal`/`pshared-cond-broadcast`/`pshared-destroy` to `pal-net.lisp`, and swap the A1 `#:sem-*` exports for `#:pshared-*`. `PTHREAD_PROCESS_SHARED = 1` on both macOS+Linux.
3. **Segment layout gains a notify block before the lanes** (C1): `[ Header(64) | mutex@64(64B) | cond@128(64B) | stop:u64@192 | pad→256 | K lanes@256 ]`. So `%lane-desc-off i = (+ 256 (* i 64))`, `%lane-data-off = (+ 256 (* lane-count 64) (* i capacity))`, `%segment-bytes = (+ 256 (* lane-count 64) (* lane-count capacity))`. (Reserve 64B each for mutex/cond — macOS mutex=64/cond=48, Linux mutex=40/cond=48 both fit.)
4. **D notification:** receiver does `pshared-lock` → `pshared-cond-wait` until a lane has data or `stop`, drain, unlock; sender enqueues then `pshared-lock`+`pshared-cond-signal`+`pshared-unlock`; shutdown sets `stop`, broadcasts, joins.

## REVISION 2026-06-14b — lane-claim via mutex (full Clasp parity, no foreign CAS, bt2 dropped)
Probing closed the A4 Clasp foreign-CAS gap a better way (owner-ratified):
5. **Lane-claim runs under the segment's pshared mutex on BOTH impls** (one-time, off the hot path: lock → scan for `owner_token`==token or ==0 → claim by `store-sap-u64` → unlock) — **no foreign-SAP CAS**. Steady-state enqueue/drain stay lock-free (single-producer `store-sap-u64` release / `load-sap-u64` acquire + `fence`), which Clasp HAS. So the ring needs **no foreign CAS at all**, and **Clasp gets full SHMEM parity** (the A4 foreign-CAS gap remains a fact but is now irrelevant). `cas-sap-u64`/`atomic-incf-sap-u64` (A3, SBCL) stay as valid-but-unused-by-ring PAL primitives (kept for a future lock-free-MPSC optimization; not deleted). **bordeaux-threads v2 atomics are NOT used** — they operate on heap `atomic-integer` objects (can't be in shared memory / cross-process), so they cannot back the ring; nothing else needs in-process atomics (YAGNI). C2's lane-claim therefore uses the mutex, not `cas-sap-u64`.
6. **`shm_open` create is per-impl** (verified opposite ABIs on macOS arm64): `#+sbcl` → `cffi:foreign-funcall-varargs` (variadic `mode` on the stack); `#-sbcl` (Clasp) → plain `cffi:foreign-funcall` (Clasp's plain funcall passes the variadic mode correctly; its varargs mispasses it). A reader conditional in `pal-net.lisp` is permitted *inside* `dds-pal/`. `shm-attach` is 2-arg non-variadic on both. This removes the Clasp/macOS attach tolerance — `run-pal-shm-test` asserts real by-name attach on every landed profile.
