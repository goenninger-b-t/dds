...
- **No reader conditionals** (`#+sbcl`/`#+clasp`) outside `dds-pal/` (runtime `(dds.pal:pal-impl-name)` dispatch instead). **No "Claude"/AI attribution** anywhere (source, docs, ADR, commits, interop README); no `Co-Authored-By`.
