...
| --- | --- | --- | --- |
| [`eprosima-typeinfo-vendor-gate.md`](eprosima-typeinfo-vendor-gate.md) | Discovery / SEDP ProxyData | SEDP vendor gate silently drops the **standard** `PID_TYPE_INFORMATION` (0x0075) from non-eProsima peers (`WriterProxyData.cpp` / `ReaderProxyData.cpp`), breaking cross-vendor XTypes type discovery | Prepared — awaiting submission |
| [`eprosima-namehash-utf8.md`](eprosima-namehash-utf8.md) | XTypes dynamic types | `get_string_from_name_hash` streams `NameHash` `uint8_t` bytes as characters (the `std::hex` is a no-op), yielding non-UTF-8 member names that break `json_serialize` (`type_error.316`) | Prepared — awaiting submission |
