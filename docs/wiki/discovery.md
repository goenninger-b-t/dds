...
- `dds.disc:node-lifecycle-change` *(node sn)* — the received dispose/unregister change at sequence number `SN` as `(kind key-hash status-flags writer-id)`, or `NIL` (lets a subscriber observe a lifecycle DATA was received + classified; the user-thread reader-side instance-state transition consumes it, recovering `writer-id` to drop the originating writer on an `:unregister`).
- `dds.disc:node-lifecycle-count` *(node)* — number of distinct dispose/unregister lifecycle DATAs the subscriber has received.
- `dds.disc:node-lifecycle-sns` *(node)* — sequence numbers of the dispose/unregister lifecycle DATAs received so far (unordered; SNs may not start at 1 against Connext) — lets the user thread drain newly-classified lifecycle changes the same way `node-sample-sns` drains data samples.
