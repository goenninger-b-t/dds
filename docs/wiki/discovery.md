...
- Accessors: `endpoint-data-guid`, `endpoint-data-topic-name`, `endpoint-data-type-name`, `endpoint-data-qos`, `endpoint-data-type-information`, `endpoint-data-zerocopy-capable` (T iff the endpoint advertised `PID_ZEROCOPY_CAPABLE` — WP-ZEROCOPY, FR-PF-3; fail-open, default NIL).
- `dds.rtps.discovery:serialize-endpoint-data` *(cursor data)* — serialize as a `ParameterList` terminated by `PID_SENTINEL` (§8.5.4 / §9.4.2.11). Emits `PID_RELIABILITY`, `PID_DURABILITY`, and `PID_LIVELINESS` (`0x001b` → `LivelinessQosPolicy {kind; Duration_t lease_duration}`, RTPS 2.5 Table 9.18 §9.6.2.2; the wire `kind` is `0=AUTOMATIC / 1=MANUAL_BY_PARTICIPANT / 2=MANUAL_BY_TOPIC`; the `lease_duration` is an RTPS-wire `Duration_t {seconds; fraction}` with `fraction` in units of `sec/2^32` and `DURATION_INFINITE = {0x7fffffff, 0xffffffff}` — the DCPS `nanosec` is converted via `dds.qos:duration-nanosec->wire-fraction` (DDSI-RTPS 2.5 §9.3.2), so an infinite lease is not misread as a finite ~0.5 s by a conformant peer), so a remote reader can RxO-match and track the writer's offered liveliness. It then emits `PID_OWNERSHIP` (`0x001f` → `OwnershipQosPolicy {kind}`, RTPS 2.5 Table 9.18 §9.6.2.2; the 4-octet `kind` is `SHARED_OWNERSHIP_QOS=0 / EXCLUSIVE_OWNERSHIP_QOS=1` per `dds_rtf2_dcps.idl`) for **both** roles, and `PID_OWNERSHIP_STRENGTH` (`0x0006` → `OwnershipStrengthQosPolicy {long value}`) for the `:writer` role **only** — `OwnershipStrengthQosPolicy` lives in `DataWriterQos`, never `DataReaderQos`, so the `endpoint-data` `role` slot gates the emission (a reader never advertises a strength). The byte form is pinned from the eProsima Fast DDS 3.6.1 oracle `interop/fastdds/captures/ownership-sedp-lo0.pcap` frame 64 (a writer `{EXCLUSIVE, strength 17}` = `1f 00 04 00 / 01 00 00 00 / 06 00 04 00 / 11 00 00 00`). It also emits the remaining RxO policies so a peer's *non-default* value is on the wire rather than decided on a local default: `PID_DEADLINE` (`0x0023`) and `PID_LATENCY_BUDGET` (`0x0027`) as `Duration_t {seconds; fraction}` (8 octets, the liveliness-lease encoding), `PID_DESTINATION_ORDER` (`0x0025`) as a 4-octet `kind` (`BY_RECEPTION_TIMESTAMP=0 / BY_SOURCE_TIMESTAMP=1`), and `PID_PRESENTATION` (`0x0021`) as `{access_scope u32 (INSTANCE=0/TOPIC=1/GROUP=2); boolean coherent; boolean ordered}` padded to 8 — all four pinned from RTPS 2.5 Table 9.14 §9.6.2.2 and verified against tshark's independent RTPS dissector on our own emission (it labels + decodes all four). Then `PID_TYPE_INFORMATION` is emitted only when present, and `PID_ZEROCOPY_CAPABLE` (`0x8041`, vendor; a single octet `1`) only when the endpoint is WP-ZEROCOPY-capable (its node has a Zero-Copy pool — default OFF, R6/ADR 0014); both are elided otherwise (peers skip unknown PIDs — fail-open).
- `dds.rtps.discovery:parse-endpoint-data` *(cursor role)* — parse a SEDP `ParameterList` into an `endpoint-data` struct, or `NIL` if truncated. Bounds-checked. The required `ROLE` (`:writer` / `:reader`) seeds the QoS defaults an **absent** parameter must assume (RTPS 2.5 §9.4.2.11.2): a DCPSPublication defaults RELIABILITY to RELIABLE, a DCPSSubscription to BEST_EFFORT (DDS 1.4 §2.2.3). RTI Connext elides default-valued PIDs, so a reliable Connext writer carries **no** `PID_RELIABILITY` — parsing it with reader defaults would fail the RxO check and silently prevent the match. `PID_DATA_REPRESENTATION` is likewise defaulted, when **absent**, to the **wire** default `[:xcdr1]` (DDS-XTypes 1.3 §7.6.3.1.1) — a stock `@final` peer elides its default-valued PID exactly as it does `PID_RELIABILITY`, so an absent PID means XCDR1, **not** the `make-*-qos` local accept/offer default `[:xcdr2 :xcdr1]`; seeding the local default made our `[:xcdr2]` writer *falsely* match an XCDR1-only reader (it sent XCDR2, the peer silently dropped it, and no `OFFERED_INCOMPATIBLE_QOS` ever fired — the direction-A silent no-match). With the wire default in place, our `[:xcdr2]` writer correctly reports the incompatibility, and an `[:xcdr1]`-configured writer matches and delivers to that reader. `PID_LIVELINESS` is parsed into the QoS `liveliness` kind + `liveliness-lease`; an unknown kind keeps the default `:automatic` and a `PID_LIVELINESS` whose length is not 12 is ignored (never read out of bounds, NFR-SEC-POSTURE) — a malformed liveliness PID must never false-REJECT a valid match. `PID_OWNERSHIP` is parsed into the QoS `ownership` kind (`1`→`:exclusive`, otherwise `:shared` — an unknown code never REJECTs) and `PID_OWNERSHIP_STRENGTH` into `ownership-strength` (a signed `long`); each is read only when its length is exactly 4, else ignored (NFR-SEC-POSTURE). `PID_DEADLINE` / `PID_LATENCY_BUDGET` (`Duration_t`, read only at length 8), `PID_DESTINATION_ORDER` (4-octet `kind`; an unknown code keeps the default `:by-reception-timestamp`), and `PID_PRESENTATION` (`{scope; coherent; ordered}`, read at length ≥ 6) are parsed into their QoS slots so `qos-rxo-compatible` decides all **nine** RxO policies on the **wire** value, not a local default. These four were RxO-checked yet previously never advertised or parsed, so a peer's non-default `deadline` / `destination-order` / `presentation` was invisible — a false-match, or (on defaults) a false-reject (RTPS 2.5 Table 9.14; the audit sibling of the data-representation absent-default fix). The `role` of the parsed struct is set from `ROLE`.
...
- `dds.disc:announce-endpoints` *(node)* — send the node's local publications (SEDP publications writer) and subscriptions (SEDP subscriptions writer) to every discovered participant's metatraffic unicast locator (§8.5.4). Also drives the periodic standalone user-data `HEARTBEAT` (`%push-heartbeat`, see below) beside `tl-sweep` / `%lease-sweep` / `%liveliness-sweep` on the announce cadence.
- `%push-heartbeat` *(node)* — writer side: emit a **periodic standalone non-final `HEARTBEAT`** (no new DATA) to each matched reader on the announce cadence (RTPS 2.5 §8.4.2.2: a reliable Writer must periodically inform each matching reliable Reader of the availability of a sample; Reliable StatefulWriter T7 transition §8.4.9.2.7). This closes the **lost-final-sample** edge: when a reliable writer's final sample's DATA was lost and no further write follows, nothing else re-prompts the reader to NACK, so the gap is never repaired; the periodic HEARTBEAT keeps reliability live and triggers the existing ACKNACK repair path. Non-final so it solicits an ACKNACK — a *complete* reader answers with a positive ACKNACK that triggers no resend. A no-op on an empty HistoryCache, no user writer, or no matched readers.
...
- `dds.disc:enable-publisher` *(node &key max-samples max-blocking-ns)* — give the node a reliable user writer (KEEP_ALL) and install the writer-side data-plane hooks (retransmit on ACKNACK; resend named fragments on NACK_FRAG). Call after `add-local-writer`. `:max-samples` (RESOURCE_LIMITS `max_samples`; `nil` = unlimited, the default — byte-identical to before) bounds the KEEP_ALL HistoryCache; `:max-blocking-ns` (RELIABILITY `max_blocking_time` in ns; `nil` = never block) makes `publish-sample` / dispose / unregister **block up to that long on a full bounded cache, then return `RETCODE_TIMEOUT` (`:timeout`)** — DDS-standard block-up-to-`max_blocking_time` backpressure (WP-ASYNC-FLOW, FR-PF-2/FR-QOS, ADR 0016 §Backpressure). With `max_samples` `nil` the cache never fills, so `max_blocking_time` has no effect.
...
- `dds.disc:publish-sample` *(node payload)* — publish an opaque `SerializedPayload` on the node's user writer: add it to the writer HistoryCache, then push DATA (or DATA_FRAGs + HEARTBEAT_FRAG) + HEARTBEAT to matched peers (FR-RTPS-8). Returns `t` normally, **or `:timeout`** if the writer's bounded cache was full and `max_blocking_time` elapsed (WP-ASYNC-FLOW backpressure; only a writer with a finite `:max-samples` + `:max-blocking-ns` can return `:timeout`, so the default path is byte-identical). On `:timeout` nothing was added or pushed (the cache is intact).
- `dds.disc:dispose-instance` *(node key-hash)* / `dds.disc:unregister-instance` *(node key-hash)* — add a no-payload **dispose** / **unregister** change for the instance named by `KEY-HASH` (16 octets) to the user writer, then push it like `publish-sample` (StatusInfo Disposed/Unregistered, RTPS 2.5 §9.6.4.9). The lifecycle DATA occupies a real SN, so it is reliably ACKNACK-repairable. Returns the change SN, **or `:timeout`** under the same block-up-to-`max_blocking_time` backpressure (ADR 0016 §Backpressure).
- `dds.disc:node-lifecycle-change` *(node key)* — the received dispose/unregister change for the composite `(GUID . SN)` `KEY` (see `node-lifecycle-sns`) as `(kind key-hash status-flags writer-id source-guid)`, or `NIL`. The lifecycle store is keyed **by source GUID then SN** (a SequenceNumber is unique only within one writer GUID, §8.3.5.4), so two writers sharing EntityId `0x102` disposing different instances at the same raw SN do not clobber each other. Lets a subscriber observe a lifecycle DATA was received + classified; the user-thread reader-side instance-state transition consumes it, recovering `writer-id` to drop the originating writer on an `:unregister` and the full `source-guid` to clear ownership of only the exact disposing writer.
- `dds.disc:node-lifecycle-change-by-sn` *(node sn)* — the lifecycle change of **any** received dispose/unregister whose RTPS SN equals `SN`, or `NIL` — a single-writer convenience over the GUID-then-SN store (mirrors `node-sample-by-sn`), for tests/diagnostics that know only the SN.
- `dds.disc:node-lifecycle-count` *(node)* — number of distinct dispose/unregister lifecycle DATAs the subscriber has received (summed over every writer's inner SN map).
...
- **DEADLOCK CONTRACT for both collectors:** `PENDING-P` runs **under the node lock**, which is a plain **non-recursive** mutex. A predicate that calls a public lock-taking node accessor **self-deadlocks the drain**, and with it every `take_samples` on the participant. Use the lock-free bodies instead: `dds.disc:node-reader-matches-writer-p-unlocked` and `dds.disc:node-reader-join-watermark-unlocked` (same rationale as `%count-eligible-drainers`, which already ran under the lock).
- `dds.disc:node-reader-matches-writer-p-unlocked` *(node reader-entity-id writer-guid)* / `dds.disc:node-reader-join-watermark-unlocked` *(node reader-entity-id writer-guid)* — the lock-free bodies of the correspondingly-named public accessors. **Caller holds the node lock.**
