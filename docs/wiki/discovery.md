...
- `dds.disc:node-lifecycle-change` *(node sn)* — the received dispose/unregister change at sequence number `SN` as `(kind key-hash status-flags)`, or `NIL` (lets a subscriber observe a lifecycle DATA was received + classified; the reader-side instance-state transition is a follow-up).
