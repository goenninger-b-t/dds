...
| `dds.xport.shmem:seg-name-for-guid` | function | `(guid domain)` — the deterministic receive-segment name a peer's 12-octet GUID prefix maps to **in `domain`**. A sender derives the destination segment name from the remote participant's prefix and its own domain, so the discovery layer addresses a same-host peer with `make-shmem-locator :name (seg-name-for-guid remote-prefix domain)`. **The domain is part of the segment identity and must be (ADR 0099):** a shm name is a process-global OS object while a DDS domain is an isolation boundary (DDS 1.4 §2.2.1.2.2), so without it two participants sharing a GuidPrefix in different domains resolve to the *same* segment. UDP never had this problem because its port range is computed from `domain_id` — the port *was* the isolation, and SHMEM inherited no equivalent. ⚠️ The token still folds only prefix octets 0..7, so two prefixes differing only in octets 8..11 collide within one domain — pre-existing, tracked, not fixed. |
| `dds.xport.shmem:run-shmem-attach-cache-race-test` | function | `()` — drives six threads at the attach cache simultaneously and asserts every thread got the **identical** dest per name. **ADR 0100:** `%attach-for` read *and* wrote the shared `attach-cache` with no lock, on **every** send, from four threads (publisher, async sender, the receiver thread's ACKNACK repair, the flow scheduler). That corrupted the table (SBCL: `failed AVER: (= HWM …)`) and leaked a segment attach when two threads raced the same new peer — both silently absorbed into a UDP fallback by the send self-guard. Every access is now under `attach-lock`; the measured cost is **+18 ns/send, 0 B**. Falsified: **5 of 8 fail** with the lock removed. |
