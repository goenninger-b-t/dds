...
| `dds.cdr:make-encapsulation-header` | Write the 4-octet header (2-octet identifier in network order + 2-octet options, sender sets options to 0) and reset the cursor's CDR alignment origin to the byte after it (RTPS 2.5 §10.2). Returns the cursor. |
| `dds.cdr:parse-encapsulation-header` | Read and validate the 4-octet header, reset the alignment origin past it, and return `(values representation-keyword options)`; bounds-checked. |
