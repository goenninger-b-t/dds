...
| `dds.cdr:encapsulation-id-for` | The LE encapsulation key a TX SerializedPayload carries, given a data-representation **and the type's extensibility**. DDS-XTypes 1.3 Table 60 (§7.6.3.1.2) keys the identifier on *both*, which is easy to miss: under encoding version 2, FINAL is `CDR2_LE 0x0007`, APPENDABLE is `D_CDR2_LE 0x0009`, MUTABLE is `PL_CDR2_LE 0x000b`; under version 1, FINAL and APPENDABLE both take `CDR_LE 0x0001` (rule (29) serializes APPENDABLE AsFinal there, so there is no DHEADER for a distinct id to announce) and MUTABLE takes `PL_CDR_LE 0x0003`. **The id must agree with the framing the codec emits** — labelling a delimited body `0x0007` tells a conformant peer there is no DHEADER, and it will read the DHEADER's four octets as the first member. This is the single definition of that mapping; it previously existed twice (the DCPS write path and the shapes harness), which is how one copy came to be corrected for APPENDABLE while the other silently was not. |
...
| `dds.cdr:string-to-utf8-octets` | Encode a string to a fresh heap octet vector of its raw UTF-8 bytes — **no length prefix, no NUL** (unlike `cdr-put-string`). For callers that need standalone UTF-8 octets for a byte sink, e.g. the logging service's RFC 5424 syslog datagram (FR-LOG-8). Shares the encode byte patterns with `cdr-put-string` via the internal `%do-utf8-octets` macro (single definition, inlined — the codec's hot-path code is unchanged). Off the measured hot path. |
...
| `dds.core.buffer:cursor-reuse` | `(c buffer)` — **the zero-allocation constructor** for a path that runs per datagram or per write. Returns `C` reset (position 0, origin 0, little-endian) when it already reads `BUFFER`, else a fresh cursor. A cursor is a six-word structure, so consing one per datagram is real cost: this campaign found the same 48 bytes at **four** sites (the inbound datagram parse, the ACKNACK reply build, the TX send fast path, the TX payload serializer). **The `EQ` test is the safety property** — a caller arriving with a different buffer gets a fresh cursor and pays the allocation it would have paid anyway, never a cursor pointing into the wrong buffer. *Whose* cursor `C` is remains the caller's problem, and the four sites answer it differently: one per receiver **thread** (private by construction), one beside the TX scratch buffer (no more shared than that buffer), one per DataWriter (safe because the payload pool hands each concurrent writer a **distinct** buffer, so the `EQ` test separates them). |
