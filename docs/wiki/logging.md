...
Drops are **reported, never printed** (FR-LOG-6), through the logger-scoped status machinery: a `get_*_status`-style **snapshot** — `logger-shed-counts`, per-severity counts indexed by RFC 5424 number; a **vendor status bit** `+log-shed-status+` (bit 24, clear of the OMG 0–14 status range) surfaced as a **changed flag** (`logger-shed-status-changed-p`, cleared by `logger-reset-shed-status` — the read-then-reset shape); a **push listener** — `logger-set-shed-listener` installs a callback fired (outside the lock) as `(fn severity-number cumulative-count)` on each shed; and a **WaitSet-attachable condition** — `logger-shed-condition` is a `GuardCondition` triggered on each shed, so a consumer can `attach-condition` it to a `dds.dcps:make-wait-set` and `wait-set-wait` for shed events instead of polling or taking a callback (the logger is not itself a DCPS entity, so a `GuardCondition`, not a `StatusCondition` proper). `logger-reset-shed-status` clears both the changed flag and the condition trigger. `close-logger` stops and joins the worker (draining the ring's remainder) before deleting the participant.
