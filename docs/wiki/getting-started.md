...
```
gate-mem: COPY   allocation = 719.6 bytes/sample (ceiling 750, NFR-MEM target 0)
gate-mem: RETURN allocation = 513.3 bytes/sample (ceiling 545, NFR-MEM target 0)
gate-mem: PASS — no regression. Returning the loan saves 206.3 B/sample; still 513 above the target of ZERO.
```
