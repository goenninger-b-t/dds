...
```
gate-mem: COPY   allocation = 705.4 bytes/sample (ceiling 730, NFR-MEM target 0)
gate-mem: RETURN allocation = 496.9 bytes/sample (ceiling 525, NFR-MEM target 0)
gate-mem: PASS — no regression. Returning the loan saves 208.5 B/sample; still 497 above the target of ZERO.
```
