...
```
gate-mem: COPY   allocation = 796.0 bytes/sample (ceiling 835, NFR-MEM target 0)
gate-mem: RETURN allocation = 594.1 bytes/sample (ceiling 625, NFR-MEM target 0)
gate-mem: PASS — no regression. Returning the loan saves 201.9 B/sample; still 594 above the target of ZERO.
```
