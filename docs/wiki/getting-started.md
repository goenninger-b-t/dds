...
```
gate-mem: COPY   allocation = 1329.2 bytes/sample (ceiling 1360, NFR-MEM target 0)
gate-mem: RETURN allocation = 1120.3 bytes/sample (ceiling 1155, NFR-MEM target 0)
gate-mem: PASS — no regression. Returning the loan saves 208.9 B/sample; still 1120 above the target of ZERO.
```
