...
(K−1)** in `%deliver-user-marker` counts all K co-located holders on the receiver thread BEFORE the marker is
drainable (so reader-A's `return-loan` can never free a slot reader-B still views; the slot frees only at the
true refcount 0); a **mid-stream-joiner ZC high-water freeze at MATCH time** (atomic with `%reader-route-add` under the node lock; the demux `{route-snapshot, bump, store}` shares that lock) so a joiner never drains a marker delivered before it joined — closing the `[freeze,route-add]` window a registration-time freeze would leave open;
and a **secured store-purge-defer** (per-`(guid,SN)` return-count) frees the shared decode handle only after ALL
K readers return (no early-return sample-loss). K=1 (N=1 / different-topic) is byte-identical.
