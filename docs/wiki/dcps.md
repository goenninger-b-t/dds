...
| Symbol | Description |
|---|---|
| `dds.dcps:create-participant` (`&key domain qos advertise-address peers autonomous`) | Open the RTPS engine (a multicast disc-node) for `domain`, install the status hooks, start the receiver, return an enabled `domain-participant`. Returns **`(values participant status)`** (ADR 0064): if the engine cannot be opened — the OS refuses a socket option, the SPDP multicast join, or the SHMEM segment — it returns `(values nil status)` **instead of signalling**. DDS 1.4 `create_participant` likewise returns a NULL handle on failure, so the `nil` primary value is the conformant shape; **callers must check it**. `:autonomous t` (S7, ADR 0056) starts a background announcer thread so discovery/matching/aging need no `spin`; it is stopped + joined on `delete-participant`. The announcer's cadence and the leaseDuration we announce to peers come from the **DISCOVERY_CONFIG** vendor-extension QoS (`qos-discovery-announce-period`, default 1 s; `qos-discovery-lease-duration`, default 100 s — see the [QoS](qos.md) page); both are changeable via `set_qos` and apply **live**. |
...
| `dds.dcps:create-datawriter` (`pub topic &key qos`) | Register a local writer in the engine on the topic's name/type with `qos`. **N non-secured writers per participant** (WP-N-ENDPOINT-S1, ADR 0048): each gets a distinct `EntityId` + its own HistoryCache; a 2nd writer under a flow-controller is supported (Slice S1b — per-writer flow-state) and a 2nd secured writer is supported (Slice S3). The endpoint kind (`WITH_KEY`/`NO_KEY`) is selected from the topic type's keyed-ness. Returns **`(values writer status)`** (ADR 0064): a local `check_create_datawriter` AccessControl deny (DDS-Security §8.4.2.4) returns `(values nil :not-allowed-by-security)`, and an exhausted 1-octet entity-key space (255 writers/participant, RTPS §9.3.1.2) returns `(values nil :out-of-resources)`, **instead of signalling** — callers must check the `nil` primary. |
| `dds.dcps:create-datareader` (`sub topic &key qos`) | Register a local reader (N readers per participant on **distinct** topics, each a distinct `EntityId`; WP-N-ENDPOINT-S2/S4). `topic` may be a `topic` or a `content-filtered-topic`. The endpoint kind (`WITH_KEY`/`NO_KEY`) is selected from the topic type's keyed-ness. N **loan-capable** (secured or ZC) readers on distinct topics are supported (S4 — each decodes under its own tier; the ZC marker demuxes per reader). Returns **`(values reader status)`** (ADR 0064): a local `check_create_datareader` AccessControl deny (DDS-Security §8.4.2.5) returns `(values nil :not-allowed-by-security)`, and an exhausted 1-octet entity-key space (255 readers/participant) returns `(values nil :out-of-resources)`, **instead of signalling**. Two or more **SAME-topic** readers are supported — NON-loan (Slice 2c-1 — route-add-all: both receive the writer's full stream, no cross-consumption) AND **loan-capable** (secured / ZC; Slice 2c-3 — the ADR-0017 refcount-per-reader UAF is closed via the demux-time `%zc-bump` by (ELIGIBLE−1), `ELIGIBLE` = the route members that will drain the marker (`SN > join-watermark`) so a frozen joiner is excluded — the joiner high-water freeze + the secured store-purge-defer + the §17.7 out-of-order-across-join leak fix, WP-DDS-ZC-REFCOUNT-LEAK). |
...
| `dds.dcps:enable` (`entity`) | `Entity::enable` (DDS 1.4 §2.2.2.1.1.7) — transition to enabled; idempotent (`:ok`), `:precondition-not-met` if the factory-parent is still disabled. A **disabled** entity's data operations — `write-sample`, `register-instance`, `dispose-instance`, `unregister-instance`, `loan-sample`, `write-loaned`, `read-samples`, `take-samples` — return `:not-enabled`; and (ADR 0060) a disabled DataWriter/DataReader is also **invisible on the wire**: it is registered with the engine but is NOT SEDP-announced and cannot be matched, so it truly *does not communicate* — `enable()` releases it onto the wire, after which it matches and data flows normally. The NOT_ENABLED-safe set (`get_qos`/`set_qos`/`enable`/`get_statuscondition`/`set_listener`) stays live. |
...
| `dds.dcps:read-instance` / `take-instance` (`dr handle &key states where`) | `DataReader::read_instance` / `take_instance` (§2.2.2.5.3.4/.5, S5) — read (leave) / take (remove) only the samples of the instance named by `handle`; `+retcode-bad-parameter+` when `handle` names no known instance. |
| `dds.dcps:read-next-instance` / `take-next-instance` (`dr previous-handle &key states where`) | `DataReader::read_next_instance` / `take_next_instance` (§2.2.2.5.3.6/.7, S5) — the samples of the next instance after `previous-handle` in the reader's stable instance order (`+instance-handle-nil+` = the first instance); an empty list when none remains. |
| `dds.dcps:read-next-sample` / `take-next-sample` (`dr`) | `DataReader::read_next_sample` / `take_next_sample` (§2.2.2.5.3.2/.3, S5) — the next `NOT_READ` sample in order (any instance), read/taken and returned, or `NIL` (NO_DATA). |
| `dds.dcps:get-matched-subscriptions` / `get-matched-publications` (`dw` / `dr`) | `DataWriter::get_matched_subscriptions` / `DataReader::get_matched_publications` (§2.2.2.4.2.10 / §2.2.2.5.2.10, S5) — the 16-octet handles (remote GUIDs) of the DataReaders / DataWriters currently **matched** (RxO-compatible) to this endpoint. |
| `dds.dcps:get-matched-subscription-data` / `get-matched-publication-data` (`dw handle` / `dr handle`) | `get_matched_subscription_data` / `get_matched_publication_data` (§2.2.2.4.2.11 / §2.2.2.5.2.11, S5) — the `Subscription`/`PublicationBuiltinTopicData` of the matched remote named by `handle`, or `NIL` (BAD_PARAMETER) when it names no matched remote. |
| `dds.dcps:write-w-timestamp` (`dw sample sec nanosec`) | `DataWriter::write_w_timestamp` (§2.2.2.4.2.11, S5.T4) — write `sample` stamped with the given `Time_t` source timestamp instead of the reception time; an **INFO_TIMESTAMP** submessage (RTPS 2.5 §9.4.5.9) carries it before the DATA, so a matched reader's `SampleInfo.source_timestamp` reflects it. `dispose-w-timestamp` / `unregister-instance-w-timestamp` are the lifecycle equivalents (§2.2.2.4.2.9/.8); `writedispose` (`dw sample &optional sec nanosec`) is a Connext-compatible extension = write-then-dispose. A **plain** `write-sample` stamps the **current wall-clock** (`dds.pal:realtime-ns`, `clock_gettime(CLOCK_REALTIME)`) — DDS `write` ≡ `write_w_timestamp(now)` — so **every** DCPS DATA carries an INFO_TS and a reader always sees a populated `SampleInfo.source_timestamp`. Our reader also **parses** a peer's INFO_TS on receive, so `source_timestamp` is populated from Connext / Fast DDS traffic (which always send one). (The low-level `dds.disc:publish-sample` path defaults to no INFO_TS — unchanged.) |
...
| `dds.dcps:acknowledge-sample` (`dr si`) | **Vendor extension (ADR 0090 A3b; DDS 1.4 defines no application acknowledgment).** Tell the writer named by `si`'s `publication-handle` that the **application** has processed the sample at `si`'s `sequence-number`, so it may stop retaining it; emits one `APP_ACK` unicast to that writer's participant. Returns `:ok` (including for a repeat — it is idempotent), `:not-enabled`, `:bad-parameter` (no publication handle), or `:precondition-not-met` when ACKNOWLEDGMENT_KIND is not `:application-explicit` **or the application never read/took that sample**. That last refusal is deliberate: a writer that believes a sample acknowledged may purge it, so an unverifiable acknowledgment is declined rather than granted. |
| `dds.dcps:acknowledge-all` (`dr`) | Acknowledge every sample this reader's application has **accessed** and not yet acknowledged, one `APP_ACK` per writer. Returns `:ok` / `:not-enabled` / `:precondition-not-met`. **Accessed, not received** — acknowledging everything the reader holds would acknowledge exactly the samples the feature exists to protect. See [qos](qos.md#acknowledgment_kind-vendor-extension-adr-0090--and-why-it-is-rxo-checked-by-equality). |
| `dds.dcps:return-loan` (`dr loans`) | `DataReader::return_loan` — accepts **three** kinds now (ADR 0093): a `flatdata-view` (ZC), a secured decode handle, and the plain **`cached-sample` wrappers `take-samples` returns**. A wrapper that *wraps* a loan delegates to the matching arm first, so returning it can never strand the loan it wrapped. Release each loaned `flatdata-view` (`%zc-release` the slot, recycle the view). **Invalidates the view's cache entry**: since `read-loaned` leaves samples in the cache, returning a view also drops its cached sample, so a returned loan is **never re-read** (reading after return is a no-op, never a stale read of the next sample's recycled-slot bytes). **Idempotent / double-return-safe.** A leaked loan pins a slot → the writer's pool gracefully falls back to non-ZC. Reader-close (`delete-participant`) returns every outstanding loan (no leaked refcount). |
...
| `dds.dcps:sample-info-publication-handle` (`si`) | The **16-octet GUID of the remote DataWriter** that wrote this sample (DDS 1.4 §2.2.2.5.4; this stack's `InstanceHandle_t` for a remote endpoint is its GUID). Populated on every delivery path since ADR 0090 A3b, which needs it: `acknowledge-sample` identifies a sample by *(writer, sequence number)*, and a sequence number is unique only within one writer (RTPS 2.5 §8.3.5.4). **Aliased, not copied** — it costs 0 B/sample and the underlying object is never mutated. |
...
### Returning taken samples — the copy path is a loan too (ADR 0093)
...
(let ((samples (dds.dcps:take-samples dr)))
  (unwind-protect
    ...
    (dds.dcps:return-loan dr samples)))   ; recycles the wrappers; omit it and you just lose the win
```
...
- **A returned sample is *gone*, not stale-readable.** `return-loan` drops it from the reader cache first,
  so reading a returned sample finds nothing rather than the *next* sample's recycled fields.
- **Only an explicit return recycles.** A KEEP_LAST eviction never does — `read` is non-destructive, so the
  application may still hold that wrapper, and recycling it there would hand its struct to the next sample
  mid-read. The explicit return is the only proof the application is finished with it.

Toggle with `dds.dcps:*rx-wrapper-pool-enabled*` (default on); the per-reader pool depth is
`dds.dcps:*rx-wrapper-pool-capacity*` (default 64 — steady state needs one; the cap only matters for a
burst, and beyond it a returned wrapper is dropped for the GC rather than parked).
...
| `dds.dcps:+status-application-acknowledgment+` (bit 28) | A matched reader's **application** acknowledged one or more of this writer's samples (ADR 0090). Bit 27 stays **reserved** — see `statuses.lisp`. |
| `dds.dcps:get-application-acknowledgment-status` (`dw`) | Snapshot: `-total-count` (+ `-change`), `-last-subscription-handle`, `-last-sequence-number`, `-app-unacked-sample-count`. ⚠️ **`app-unacked-sample-count` is not the ADR 0089 `unacked-sample-count`** — the protocol window can be empty while this one is full, because RTPS acknowledges on receipt and the application acknowledges on processing. Reads all zeros under `ACKNOWLEDGMENT_KIND :protocol`, which is honest rather than inert: the writer genuinely has no application-level knowledge. |
| `dds.dcps:on-application-acknowledgment` (`l writer status`) | Fires when a matched reader's application acknowledges. **The only callback that reports "the application has processed it"** — `on_publication_matched` says a reader exists and `on_reliable_reader_activity_changed` says its RTPS layer is acknowledging; both can be true while the subscribing application has consumed nothing. Not edge-triggered or episode-gated (unlike the cache status): an application acknowledgment is an *event* the peer chose to send, not a level that moves twice per sample. |
| `dds.dcps:+status-application-acknowledgment-overdue+` (bit 29) | A sample has gone un-application-acknowledged for longer than the writer's `ACKNOWLEDGMENT_DEADLINE` (ADR 0090 A4). |
| `dds.dcps:get-application-acknowledgment-overdue-status` (`dw`) | Snapshot: `-total-count` (+ `-change`), `-last-subscription-handle` (the **laggard** reader), `-oldest-unacknowledged-sequence-number`, `-app-unacked-sample-count`. |
| `dds.dcps:on-application-acknowledgment-overdue` (`l writer status`) | Fires when the deadline elapses with an un-acknowledged backlog. **The only way a stalled application becomes visible**: the peer's RTPS layer keeps ACKNACKing, so `on_reliable_reader_activity_changed` reports it ACTIVE throughout. ⚠️ **Reports and never purges** — releasing the samples would discard data no application processed. |
