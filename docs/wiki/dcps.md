...
| `dds.dcps:write-loaned` (`dw loan`) | Publish the loaned sample. Slot-backed: materialise the RETAINED payload (one slot→heap copy — it serves retransmission / non-ZC / extra-ZC destinations), `%zc-loan-commit` the slot, then publish with the change **carrying the pre-committed slot**: the send site emits the slot's 20-octet ref to the FIRST ZC-eligible destination with **no payload→slot copy** (the end-to-end 0-copy leg, ADR 0042 §5), and every other case is served from the retained payload exactly as `write-sample`. Fallback: exactly `write-sample`. Returns `+retcode-ok+` / `+retcode-timeout+` (same backpressure as `write-sample`). **Idempotent** (a re-write, or a write after `discard-loan`, is a no-op). |
