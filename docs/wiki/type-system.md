...
- **`dds.types:type-support-sample-pool-alloc`** / **`dds.types:type-support-sample-pool-free`** — loan / return a pre-allocated sample for zero-per-sample-allocation deserialize. ⚠️ Present but **not used by the engine**: the per-type pool is shared across readers, so DCPS keeps its own per-reader pool instead.
- **`dds.types:type-support-copy-into`** — `(src dst) → dst`, filling `dst` **in place** from `src`, **deep for every reference slot** (a later in-place mutation of `src` is never visible through `dst`); `nil` for a `:flatdata t` type, whose sample is a buffer rather than a struct (ADR 0105). Emitted per type as `copy-into-<name>`. It is what lets a *non-loan* take hand the application its own copy and recycle the middleware's decoded struct immediately — the mechanism behind `take-into` / `read-into`. Sequence and string members go through `dds.gen::%copy-seq-into` (`src/dds-gen/runtime.lisp`), which **reuses the destination's storage only on an exact length + element-type match** and otherwise allocates exactly one fresh sequence: a longer destination cannot be made to read short without allocating, because these slots are plain `vector`/`string` with no fill pointer. Growing to a ceiling instead of reallocating is ADR 0105 §5 (slice 2). The element-type test is a **correctness** guard, not a tuning knob — `replace`-ing a `(signed-byte 32)` source into an `(unsigned-byte 8)` destination signals a `TYPE-ERROR` after partially overwriting it, and no condition may escape `src/`.
- **`dds.types:type-support-sample-p`** — the generated `<name>-p` structure predicate (ADR 0105). Bound for **every** type, FlatData included. It exists because `copy-into` is fully typed: `take-into` validates each application-supplied destination element with it and returns `PRECONDITION_NOT_MET`, where an unchecked wrong-typed element would make the copier signal a `TYPE-ERROR` out of the middleware — and the natural application mistake produces one, since `(make-array 32)` is a vector of *zeros*, not of samples. It is a **capability the caller may not assume is bound**, exactly as `copy-into` is not: test it, do not call it blind.
