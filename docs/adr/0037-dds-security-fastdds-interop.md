...
1. **§9.3.2 0xC2→0xC7 secure-builtin EntityId pairing is computed in three places** (the secure-SEDP
   ...
   **RESOLVED — WP-SECURITY-CARRIES-BATCH (2026-07-03).** Factored into ONE shared source in `dds.rtps.discovery`
   (below both packages): `builtin-complementary-eid` (the §9.3.1.2 low-byte 0xC2↔0xC7 EntityKind flip) +
   `secure-builtin-writer-eid-p` (the §7.4.5 writer gate). All three sites now delegate
   (`%secure-reader-eid-for-writer`, `%secure-sedp-reader-for-writer`, the DW/DR complementary-endpoint branch) —
   byte-identical (the explicit-constant cond and the arithmetic produce the same values); existing discovery /
   origin-auth / crypto-manager tests green UNCHANGED.
2. **No dedicated our-to-our test counts secure-builtin ACKNACKs on the wire.** The T9 reliability
   ...
   **RESOLVED — WP-SECURITY-CARRIES-BATCH (2026-07-03).** `run-secure-builtin-acknack-count-test` drives
   `%on-secure-builtin-heartbeat` and captures the emitted ACKNACK on the wire (datagram sink): a HEARTBEAT [1,3]
   to an empty builtin reader emits EXACTLY ONE PROTECTED (SEC_PREFIX) datagram, the disc-node ACKNACK counter
   ticks by 1, and the decoded inner submessage IS a secure-SEDP ACKNACK (0xff0003c7←0xff0003c2) NACKing THREE SNs
   (numBits 3); after recording SNs 1..3 a second HEARTBEAT emits an ACKNACK NACKing ZERO (numBits 0) — so the
   reliability count responds to reader state (non-vacuous). §8.5.1.8/.9, both impls.
...
|---|---|---|
| **1 — 0xC2→0xC7 pairing computed in three places (DRY)** | **RESOLVED (WP-SECURITY-CARRIES-BATCH, 2026-07-03)** | Factored into ONE shared source in `dds.rtps.discovery` (below both packages, so `dds-disc` need not import `dds-dcps`): `builtin-complementary-eid` (§9.3.1.2 low-byte 0xC2↔0xC7 flip) + `secure-builtin-writer-eid-p` (§7.4.5 gate). All three sites (`secure-sedp.lisp` `%secure-reader-eid-for-writer`, `crypto-manager.lisp` `%secure-sedp-reader-for-writer` + the DW/DR complementary-endpoint branch) now delegate — byte-identical; discovery / origin-auth / crypto-manager tests green UNCHANGED both impls. |
| **2 — no dedicated our-to-our test counts secure-builtin ACKNACKs** | **RESOLVED (WP-SECURITY-CARRIES-BATCH, 2026-07-03)** | `run-secure-builtin-acknack-count-test` drives `%on-secure-builtin-heartbeat` and captures the emitted protected ACKNACK (datagram sink): exactly ONE SEC_PREFIX datagram per HEARTBEAT, the ACKNACK counter ticks by 1, the decoded inner submessage is the secure-SEDP ACKNACK NACKing 3 SNs (numBits 3) for an empty reader and 0 after receiving them (non-vacuous — the count responds to state). §8.5.1.8/.9, both impls. |
| **3 — SIGN-tier inter-submessage alignment + GMAC AAD byte-span** | **RESOLVED (WP-SECURITY-SIGN-LIVE-CONNEXT, 2026-07-03)** | The `governance-sign` tier was completed to the authenticated-but-visible posture (`discovery/liveliness/rtps/metadata`=SIGN, `data`=NONE) and driven LIVE both directions ours↔RTI Connext 7.3.1 at GOV=sign (`run-connext-interop.sh sign`): ours=pub → Connext reads 8 visible SIGN HelloWorld samples; Connext=pub → ours decodes 42 (`message="Hello world from Connext"`). The **SIGN GMAC AAD byte-span was EMPIRICALLY confirmed conformant** — a decode probe proved ours reproduces Connext's whole-RTPS common_mac under `AAD = the verbatim submessage stream, empty plaintext` (the NIST-GMAC form; the `GCM-tag-over-ciphertext` and `SecureDataHeader-prefixed-AAD` hypotheses did NOT reproduce it), so no AAD change was needed. **The removed live decode probe is now backstopped by an in-tree OUR-TO-OUR regression assertion** (`%t2-corpus-sign-aad-span`, security test suite, §8.5.1.9 / §9.5.3.3.4.3): it recomputes the GMAC over AAD = the verbatim visible submessage span (extracted from our own SIGN encode, under the CryptoHeader's own session_id/iv_suffix, empty plaintext) and equalp-asserts it equals the on-wire `common_mac`, with an off-by-one wider span asserted NOT to match — so a future codec change that widened/narrowed the SIGN AAD span FAILS a test rather than silently regressing cross-vendor SIGN. Byte-exact + deterministic; no corpus regenerated (the T2 SIGN-80 vector is unchanged). The SIGN inter-submessage 4-alignment held (Connext's + ours' verbatim streams are mod-4=0). See ADR 0040 (Slice-5c) for the three SIGN-tier reconciliations that unblocked the live run (participant-crypto advertised-kind, metadata double-wrap, data_protection=NONE gate). |
...
| **5a — zero-alloc into-buffer AEAD on the data path** | **RESOLVED (send path) — mem-covered** | ADR 0038/0039 (WP-DDS-SECURITY-ZEROALLOC-AEAD) landed the zero-alloc submessage `metadata_protection` + whole-RTPS `rtps_protection` tiers; `make mem` now COVERS the dds-disc path with the secured SEND arms at 0.0000 B/sample (`meta-send`/`rtps-send`/`rtps-recv` delta 0.0000). **The `meta-recv` residual is now RESOLVED (ADR 0039 (c), WP-ADR-SMALL-CARRIES C2, 2026-07-03):** it surfaced + fixed a REAL ~49 B/sample alloc (an RTPS-header `cursor` cons → the zero-alloc `write-header-into`); the ~176 B that remains is NOT a per-sample alloc but a one-off GC-region-boundary quantum of the cancelling `%handle-datagram` re-dispatch in the measured window (amortized ~0.16 B/sample over 400k iters), which passes the `< 1.0` mem gate. No real per-sample alloc remains on the metadata receive transform. |
| **5b — KeyMaterial GC-heap → foreign/static** | **PARTIALLY RESOLVED** | Commit `6beb08b` (WP-SECURITY-KEYMATERIAL-HARDEN) moved the KeyMaterial MASTER secrets (salt/sender/receiver) to foreign/static memory + zeroize-on-teardown (ADR-0034 Carry-4 resolved for the master slots). **RESOLVED-AS-DESIGNED for the remaining slots:** the DERIVED session-key caches (`cached-session-key`/`cached-recv-session-key`/`cached-recv-master-key`) DELIBERATELY stay GC-heap — a review caught that foreign-static-per-session_id would UNBOUNDEDLY leak un-wiped key bytes on session_id rotation (pre-auth-reachable DoS), so they are heap-by-design (re-derivable, GC-safe), with a RED regression guard (`security-test.lisp` asserting rotated keys stay GC-heap). No non-master slot should be foreign-hardened; the follow-on is closed. |
...
| **5e — per-topic `metadata_protection` selective / `pvms-bootstrap-kms` peer-loss pruning / `%dn-normalize` RFC2253 edges / `%on-secure-builtin` inner-writerId cross-check** | **RESOLVED — WP-SECURITY-CARRIES-BATCH (2026-07-03), see ADR 0040 §carry-resolutions** | All four closed piece-by-piece as independent commits: `metadata_protection` per-topic selectivity (add-local `%refine-user-protection` + `governance-effective-metadata-protection`); `%on-secure-builtin` inner-writerId cross-check (submessage-substitution defense, `disc-node-secure-sedp-decode-sender-entity`); `pvms-bootstrap-kms` peer-loss prune (`%prune-pvms-bootstrap-km`, quiesced-drop); `%dn-normalize`/`%dn-equal` RFC2253 §2-3 edges (unescaped-separator split + value un-escape + fail-closed on malformed, `run-security-dn-match-test`). None was on the P6 exit-gate path. |

**Net:** carries 1, 2, 3, 4, 5c, 5e ALL RESOLVED (WP-SECURITY-CARRIES-BATCH, 2026-07-03 — 0xC2↔0xC7 DRY, secure-builtin ACKNACK count, metadata_protection selectivity, inner-writerId cross-check, pvms-prune, `%dn-normalize` RFC2253 edges); 5a, 5b, 5d substantially resolved/validated by Slice 5b + the zero-alloc/keymaterial WPs. No OPEN Fast-DDS-era carry remains; no new large reconciliation was needed vs Connext.
