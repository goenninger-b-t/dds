...
- **Per-endpoint STATUS/listener dispatch for SAME-topic readers is a TRACKED FOLLOW-ON — 2c-1 does NOT ship it as working.** 2c-1 delivers same-topic **DATA** to ALL same-topic readers (the slice's deliverable, proven above). But the S5 status resolver (§13.1) keys the local entity by **topic-name**, so for two SAME-topic readers only ONE receives the `SUBSCRIPTION_MATCHED` / `LIVELINESS_CHANGED` / incompatible-QoS status counter + listener callback — exactly the "resolver keyed finer than topic-name" consequence §5 flags. This per-endpoint STATUS incompleteness for same-topic readers is deferred to the 2c sequence (2c-2 or a dedicated 2c-status slice), to be resolved by keying the status resolver finer than topic-name (e.g. by the matched reader `EntityId` via the delivery route, as the liveliness hook already does for its GUID→route resolution). DATA delivery to BOTH readers is complete and correct; STATUS delivery to BOTH is NOT — do not rely on same-topic per-endpoint status yet.
