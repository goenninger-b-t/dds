...
| M2 | P1 minimal RTPS + Connext Shapes interop | 🟡 **IN PROGRESS** — **RTPS wire-codec layer complete** — Header/SubmessageHeader/EntityId/SequenceNumber/SequenceNumberSet/HEARTBEAT/ACKNACK/GAP/DATA, all byte-exact + bounds-checked + spec-cited. HistoryCache + **reliable writer/reader engine** (HB/ACKNACK/GAP state machines, fault-injection tested) done. Next: SPDP/SEDP discovery + ParameterList/inlineQos codec + UDPv4 transport (PAL sockets) + byte/transport wiring, then **Connext Shapes interop** (exit gate — needs owner's Connext + tshark). |
