...
| M2 | P1 minimal RTPS + Connext Shapes interop | 🟡 **IN PROGRESS** — **RTPS wire-codec layer complete** — Header/SubmessageHeader/EntityId/SequenceNumber/SequenceNumberSet/HEARTBEAT/ACKNACK/GAP/DATA, all byte-exact + bounds-checked + spec-cited. HistoryCache (HISTORY/RESOURCE_LIMITS) done. Next: reliable writer/reader engine (HB/ACKNACK/GAP state machines + fault-injection tests) + SPDP/SEDP + UDPv4, then **Connext Shapes interop** (exit gate — needs owner's Connext + tshark). |
