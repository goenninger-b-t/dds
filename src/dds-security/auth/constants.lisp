...
;;; --- ParticipantGenericMessage / ParticipantStatelessMessage wire constants (§7.4.4 / §9.3) ---
;;; ParticipantStatelessMessage is a typedef alias for ParticipantGenericMessage (§7.4.4 IDL).
;;; Serialized CDR-LE into the SerializedPayload of a PSM DATA submessage.
;;; Field order (from Fast DDS CDRMessage::addParticipantGenericMessage + OMG §7.4.4):
...
;;;   message_class_id (CDR-LE string: u32-LE(len+1)+bytes+NUL+pad4)
;;;   message_data (DataHolderSeq: u32-LE(count) + DataHolder*)
;;; No message_aux field is serialized (Fast DDS §7.4.4 Fast DDS corroboration; NEEDS-VERIFICATION
;;; against OMG §7.4.4 IDL text — assessed absent, see spike 2026-06-25-dds-security-auth-wire-transport.md §10.1).

(defconstant +auth-message-class-id+
    (if (boundp '+auth-message-class-id+)
        (symbol-value '+auth-message-class-id+)
        "dds.sec.auth")
...
;;; For IdentityToken:         PropertySeq carries 4 string pairs; BinaryPropertySeq is empty (count=0).
;;; IMPORTANT: DataHolder in the PSM wire is CDR-LE; hash/signature inputs (handshake.lisp) use CDR-BE.
;;; These are two DISTINCT serializations of the same logical property set (§9.3.2 / §7.4.4).

(defconstant +dataholder-empty-property-seq-count+ 0
  "Sentinel count for an empty PropertySeq or BinaryPropertySeq in a DataHolder (u32-LE = 4 zero octets).")

;;; MessageIdentity field widths (§7.4.4):
(defconstant +message-identity-guid-len+ 16
  "MessageIdentity.source_guid byte length: GuidPrefix_t(12) + EntityId_t(4) (DDS-Security 1.1 §7.4.4).")
(defconstant +message-identity-sn-len+ 8
  "MessageIdentity.sequence_number byte length: int64 CDR-LE (DDS-Security 1.1 §7.4.4).")
(defconstant +message-identity-len+ 24
  "Total CDR-LE length of one MessageIdentity = GUID(16) + SN(8) (DDS-Security 1.1 §7.4.4).")

;;; --- c.pdata ParticipantBuiltinTopicData ParameterList constants (§9.3.2.1) ---
;;; c.pdata is the CDR ParticipantBuiltinTopicData of the originating participant. A replier reads
;;; PID_PARTICIPANT_GUID from it as a BIG-ENDIAN RTPS ParameterList read at offset 0 with NO
;;; encapsulation header (corroborated: Fast DDS PDP::get_participant_proxy_data_serialized(BIGEND)
;;; emits write_to_cdr_message(..., write_encapsulation=false); PKIDH::begin_handshake_reply +
;;; ParameterList::read_guid_from_cdr_msg scan PIDs from pos 0). These RTPS PIDs / EntityId are
;;; re-pinned locally because dds.security must NOT depend on dds.rtps (the dependency runs
;;; dds.rtps -> dds.security; same decoupling note as crypto/constants.lisp).

(defconstant +pid-participant-guid+ #x0050
  "PID_PARTICIPANT_GUID (RTPS 2.5 §9.6.2.2 table). Re-pinned locally; dds.security must not depend on dds.rtps.")

(defconstant +pid-sentinel+ #x0001
  "PID_SENTINEL, terminates a ParameterList (RTPS 2.5 §9.6.2.2 / §9.4.2.11). Re-pinned locally.")

(defconstant +entityid-participant+ #x000001c1
  "ENTITYID_PARTICIPANT (RTPS 2.5 §9.3.1.2): the 4-octet EntityId_t of the participant GUID, MSB-first u32.
   Re-pinned locally; dds.security must not depend on dds.rtps.")
...
(defparameter +test-ssd-pki-root+
    (uiop:pathname-directory-pathname
     (merge-pathnames
      "interop/security-secure-discovery/pki/"
      (or (uiop:getenv "DDS_REPO_ROOT")
          (asdf:system-source-directory :dds-security))))
  "Root directory of the secure-discovery test-PKI fixture (interop/security-secure-discovery/pki/).")
