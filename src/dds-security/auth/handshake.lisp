...
(defun* %parse-token (octets)
  ...
  "Parse a handshake-token from the internal tagged format. Returns NIL on malformed input.
   Bounds-checks every length before trusting wire data (NFR-SEC-POSTURE; fail-closed). ADR 0064: a malformed
   peer token is a fail-closed NIL via a local RETURN-FROM (the `bad` macrolet), never a condition — the
   function no longer signals-and-catches-its-own-errors (the old handler-case is gone); the NIL contract is
   byte-identical to callers."
  (macrolet ((bad () '(return-from %parse-token nil)))
    (let ((n (length octets))
          (pos 0))
      (labels ((ensure (need)
                 (when (> (+ pos need) n) (bad)))
