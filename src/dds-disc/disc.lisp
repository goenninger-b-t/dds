...
(defstruct* (disc-node (:constructor %make-disc-node))
  ...
  (shmem-internal-bugs 0 :type fixnum)    ; ADR 0100: count of IMPLEMENTATION-INTERNAL invariant violations (dds.pal:internal-bug-p) caught on the SHMEM send path — counted SEPARATELY from an ordinary send fault because it means a structure is already corrupt, not that a peer went away
  (shmem-internal-bug-p nil :type t)      ; ADR 0100: latched T by the first internal bug — SHMEM is disabled for this node from then on, so a possibly-corrupt attach cache is never consulted again (fail-safe: UDP still delivers)
  ...
  ;; ADR 0078 re-land: the SET of pooled buffers currently checked out (eq: octet-buffer -> t), guarded by
  ;; rx-store-pool-lock. Buffer lifetime is tracked HERE, decoupled from store entries — release is idempotent
  ;; (remhash-then-return, so a buffer already returned is a no-op, never a double pool-release) and teardown
  ;; returns exactly this set. The reverted ADR released by WALKING store entries, which double-releases when a
  ;; buffer is reachable from two entries (aliasing) — the instrumented Linux backtrace that pinned this.
  (rx-store-checkout (make-hash-table :test 'eq) :type hash-table)
...
(defun* %rx-cursor (ctx buf)
  ...
  "The datagram cursor for BUF: CTX's own cursor reset for reuse when it already reads BUF, else a fresh
   ...
   CONSTRUCTION rather than by audit. Each receiver thread (unicast / multicast / SHMEM) owns exactly
   ONE receive buffer for its whole life (start-udp-receiver's 64 KiB buffer, the SHMEM transport's
   sink), so ONE cursor per receiver thread serves every datagram it will ever handle; the EQ test is
   what makes that a property of the code rather than an assumption about the transports, and a
   caller that does hand over a different buffer simply pays the pre-existing allocation.
