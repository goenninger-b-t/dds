...
(defun* %ensure-send-scratch-pool (node)
  ...
  (%lazy-carve-pool (pool node :pool  disc-node-send-scratch-pool
                               :lock  disc-node-send-scratch-lock
                               :arena disc-node-send-scratch-arena
                               :element-bytes (+ (srtps-scratch-datagram-bytes) +srtps-scratch-overhead+)   ; a wrapped datagram + the SRTPS bracket
...
(defun* %ensure-submsg-scratch-pool (node)
  ...
  (%lazy-carve-pool (pool node :pool  disc-node-submsg-scratch-pool
                               :lock  disc-node-submsg-scratch-lock
                               :arena disc-node-submsg-scratch-arena
                               :element-bytes (+ (srtps-scratch-datagram-bytes) +submsg-scratch-overhead+)   ; a datagram + the per-submessage bracket expansion headroom
                               :capacity      *srtps-send-scratch-capacity*)))
...
(defun* %ensure-zc-overlay-scratch (node)
  ...
  (%lazy-carve-pool (pool node :pool  disc-node-zc-overlay-scratch-pool
                               :lock  disc-node-zc-overlay-scratch-lock
                               :arena disc-node-zc-overlay-scratch-arena
                               :element-bytes (+ 44 +zerocopy-pool-slot-bytes+ 3)   ; the ENCRYPT framing, the LARGER of the two tiers (ADR 0058)
                               :capacity      4)))
      ...
      (when (and (%small-change-p change)
        ...
        (let ((size (%change-submessage-size change)))
          (when (and (zerop (mod size 4))                       ; 4-aligned -> %pack-plan would NOT force a boundary after it
            ...
            (let ((mc (setf (disc-node-tx-cursor node) (dds.core.buffer:cursor-reuse (disc-node-tx-cursor node) buf)))
                  (wid (%emit-wid node)))
              (dds.rtps.message:write-header mc (disc-node-guid-prefix node))
     ...
     (let ((element-bytes (+ 44 *secured-payload-max-bytes* 3))   ; encode-serialized-payload-into exact bound (transform.lisp)
       ...
       (handler-case
           (let* ((arena (%node-arena node))   ; ADR 0095: THE participant's sub-arena, charged against the process budget
             ...
             (when pool   ; ADR 0098: a WHEN, never a RETURN-FROM — NIL is writer-ensure-payload-pool's DOCUMENTED provision-fn result, and an NLX out of this HANDLER-CASE through that function's writer lock heap-allocates the handler closure on EVERY call (16 B/call measured); a failed carve charged the SHARED arena nothing, so there is nothing to tear down (ADR 0095)
...
(defun* %ensure-secured-decode-pool (node)
  ...
  (let ((capacity (+ *secured-pool-capacity* *secured-pool-headroom*)))
    (%lazy-carve-pool (pool node :pool  disc-node-decode-pool
                                 :lock  disc-node-decode-pool-lock
                                 :arena disc-node-decode-arena
                                 :element-bytes *secured-payload-max-bytes*   ; the max recovered plaintext = decode-serialized-payload-into's ct_len bound
                                 :capacity      capacity
                                 :conditions    (or error storage-condition))
      ;; T5d: the CAPACITY-sized loan WRAPPERS, so the accepted loan conses 0 B/sample. Plain heap objects
      ;; (NOT arena/SAP); only the off-heap plaintext BUFFERS come from the static arena.
      (let ((handles (make-array capacity)))
...
(defparameter *rx-store-pool-enabled* t
  "ADR 0078 / ADR 0085 (NFR-MEM, NFR-STABILITY): master switch for the RX store-copy pool. NIL makes every
   copy-path receive allocate its store vector exactly as the pre-pool path did — byte-identical delivery at
   the pre-pool allocation cost, and no pool is ever carved. Read per receive (never latched), so it can be
   flipped at runtime without a rebuild.

   WHY THIS EXISTS. This pool's historical failure mode is a STABILITY defect, not a functional one (ADR 0078
   was reverted for Linux heap corruption). Attributing a corruption to it therefore requires a SAME-TREE A/B
   — same binary, same test order, the pool as the only variable — which is exactly what this switch buys;
   and an operator who hits such a defect in the field needs a kill-switch that is not a recompile. Does not
...
(defun* %ensure-rx-store-pool (node)
  ...
  (let ((element-bytes (+ 4 *plain-payload-max-bytes* 8))   ; deliberately the SAME bound publish-sample-into's allocating fallback uses
        (capacity *rx-store-pool-capacity*))
    (%lazy-carve-pool (pool node :pool  disc-node-rx-store-pool
                                 :lock  disc-node-rx-store-pool-lock
                                 :arena disc-node-rx-store-arena
                                 :element-bytes element-bytes
                                 :capacity      capacity
                                 :conditions    (or error storage-condition)
                                 :on-failure    (setf (disc-node-rx-store-carve-failed node) t))
  ...
  (let ((pool (or (disc-node-rx-store-pool node)
    ...
    (when (and pool (<= plen (disc-node-rx-store-element-bytes node)))
      (let ((ob (dds.pal:with-lock ((disc-node-rx-store-pool-lock node))
                  (let ((b (dds.core.arena:pool-acquire pool)))
                    (when b
                      (setf (gethash b (disc-node-rx-store-checkout node)) t))   ; track: release is exactly-once from this set
                    b))))
        (when ob
  ...
  (let ((pool (disc-node-rx-store-pool node)))
    (when pool
      (dds.pal:with-lock ((disc-node-rx-store-pool-lock node))
        ;; IDEMPOTENT, exactly-once: return to the pool ONLY if this buffer is currently checked out. A buffer
        ;; reachable from two store entries (aliasing) or released twice is a clean NO-OP — never the double
        ;; pool-release that duplicated a free-list slot and corrupted the heap (the reverted ADR's widetag-0,
        ;; pinned by the instrumented teardown backtrace). remhash returns T iff OB was present.
        (when (remhash ob (disc-node-rx-store-checkout node))
...
(defun* node-return-all-rx-store-buffers (node)
  ...
  "ADR 0078 teardown safety, the exact twin of node-return-all-loans: return EVERY pooled store-copy buffer
   ...
   teardown-arena. Each returned buffer's store entry is DETACHED (its value set to NIL) in the same
   pass: teardown-arena is about to free the pooled octet-buffer's backing STATIC vector, and a store
   entry left pointing at that octet-buffer is a live Lisp reference to freed foreign memory — the next
   GC to scavenge the still-reachable sample table walks the octet-buffer's vec slot into freed storage
   and faults (`no transport function ... widetag 0`, ADR 0078 §re-landing: the FIRST corruption
   signature; returning the buffer without detaching the entry made it worse, not better). Setting an
   existing entry's VALUE during maphash is permitted (no key is added/removed); the tables are discarded
   with the node immediately after."
  (when (disc-node-rx-store-pool node)
    (dds.pal:with-lock ((disc-node-lock node))
      (dds.pal:with-lock ((disc-node-rx-store-pool-lock node))
        ;; Return EXACTLY the checked-out set (each buffer once), NEVER by walking store entries — a buffer
        ;; reachable from two entries (aliasing) would be released twice, which is the double-release the
        ;; instrumented backtrace pinned to this function. Then empty the set.
        (maphash (lambda (ob v)
                   (declare (ignore v))
                   (dds.core.arena:pool-release (disc-node-rx-store-pool node) ob))
                 (disc-node-rx-store-checkout node))
        (clrhash (disc-node-rx-store-checkout node)))
      ;; DETACH every store entry still referencing a pooled buffer, so no live Lisp reference survives into
      ;; the static memory teardown-arena is about to free (the widetag-0 UAF).
      (maphash (lambda (guid inner)
                 (declare (ignore guid))
                 (maphash (lambda (sn v)
                            (when (typep v 'dds.core.buffer:octet-buffer) (setf (gethash sn inner) nil)))
                          inner))
               (disc-node-samples node))))
  t)
